...
const allTagsForm =
  `<ul>
    <li id="superuser-filter">
      <input id="superuser" type="checkbox" name="superuser" value="$$$"></input>
      <label for="superuser">查找已合并标签</label>
    </li>
    <li><h3>新闻分类</h3></li>
    <li>
...
// Render logo and stats link hrefs with username for getting back to previous
// query.
function renderHeaderLinks (un, tags = []) {
  if (typeof tags === 'string') tags = [tags] // might not be an array
  document.querySelector('#logo a').href += `?q=${tags.join()}&username=${un}`
  document.getElementById('stats-link').href += `?username=${un}`
}
